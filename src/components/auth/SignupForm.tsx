...
export default function SignupForm({ onSuccess }: SignupFormProps) {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { user, error: authError, suggestDemo: shouldSuggestDemo } = await signUpWithEmail(email, password);
      ...
      // Then create team and user profile via Cloud Function
      const result = await createTeamCreateUser({
        teamName,
        displayName,
        email
      });
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // Create team with Google account info
      const result = await createTeamCreateUser({
        teamName,
      ...
      console.log('Team created with Google:', result.data);
      onSuccess?.();
      ...
      <div className="mt-4">
        ...
        {!teamName.trim() && (
          <p className="text-sm text-gray-500 mt-2 text-center">
            Enter a team name to enable Google signup
          </p>
        )}
      </div>
    </div>
